...
namespace CC98.LogOn.Controllers
{
    ...
    public class AppController : Controller
    {
        ...
        /// <param name="returnUrl">操作后要返回的地址。</param>
        /// <returns>操作结果。</returns>
        [Authorize]
        [HttpGet]
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(App model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    MessageAccessor.Messages.Add(OperationMessageLevel.Success, "操作成功",
                        string.Format(CultureInfo.CurrentUICulture, "你已经成功修改了应用 {0}", model.DisplayName));
